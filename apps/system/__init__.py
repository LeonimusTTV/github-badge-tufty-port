...
TZ_MIN = -12
TZ_MAX = 14
# Whether main.py plays the boot cinematic (apps/startup). Toggled with the
# C button on this screen; main.py reads the same "system" state to decide.
DEFAULT_STARTUP_ENABLED = True
config = {"tz_offset": DEFAULT_TZ_OFFSET, "startup_enabled": DEFAULT_STARTUP_ENABLED}
...
def load_config():
    ...
    config["startup_enabled"] = bool(config.get("startup_enabled", DEFAULT_STARTUP_ENABLED))
...
ntp_fail_at = None    # badge.ticks of the last failed NTP attempt
NTP_RETRY_MS = 30000  # wait between failed NTP attempts (the call blocks)
connect_at = 0        # badge.ticks of the last wlan.connect() attempt
...
def tick_network():
    ...
    if not have_ip():
        ...
        elapsed = badge.ticks - connect_at
    ...
    if ntp_fail_at is not None:
        elapsed = badge.ticks - ntp_fail_at
...
def draw_battery(x, y, level, charging):
    ...
    # body outline
    screen.pen = gray
    screen.shape(shape.rectangle(x, y, 24, 12).stroke(1))
    # positive terminal
    screen.shape(shape.rectangle(x + 24, y + 3, 2, 6))
    # fill
    ...
    if charging:
        ...
        screen.pen = phosphor
    if fill_w > 0:
        screen.shape(shape.rectangle(x + 2, y + 2, fill_w, 8))
...
def update():
    ...
    if badge.pressed(BUTTON_UP) and config["tz_offset"] < TZ_MAX:
        config["tz_offset"] += 1
        save_config()
    if badge.pressed(BUTTON_DOWN) and config["tz_offset"] > TZ_MIN:
        config["tz_offset"] -= 1
        save_config()

    # Toggle the boot cinematic (apps/startup) with button C.
    if badge.pressed(BUTTON_C):
        config["startup_enabled"] = not config["startup_enabled"]
        save_config()
    ...
    # Footer (two lines - one row is too cramped to stay legible)
    screen.pen = gray
    screen.shape(shape.rectangle(5, 102, 150, 1))
    screen.font = small_font
    intro_state = "on" if config["startup_enabled"] else "off"
    center_text("A:sync  UP/DN:zone", 104)
    center_text("C:intro %s" % intro_state, 113)
